...
The reader of this program doesn't need to guess if the input is radians or degrees. One interesting thing you can do here is add custom arguments to existing functions. In this example, we create a new coordinate system that takes angles in “fun-degrees”
...
We've started fleshing out how modularity will work in Eve. For a while now, we've had a concept of "contexts" (formerly bags), which are just containers of facts. So far, we've only supported two contexts: a "session" context and a "global" context. This was exposed to users via `commit global` which would direct committed records to a global context accessible by any session on the server. 

Now, we've opened up the ability to send facts to and select facts from specific contexts. For instance, until now a committed `#div` was implicitly directed to a special browser context. Now, if you want to render something in the browser you'll have to tell Eve to put it there explicitly:
...
Contexts are specified after one of the three fences: `match`, `commit`, or `bind`. If no context is specified, the context is implicitly a default session context. You can even work with multiple contexts at once:

```
match (@my-context, @your-context)
  [#person name]
bind
  [#div text: {{name}}]
```

This would select every `#person` from two different contexts, essentially merging two databases.

With this mechanism in place, we can start to talk about how modularity and libraries work in Eve. For instance, I could create a custom math library, and what I would distribute is a context. Then anyone who wants to use my custom math would do the following:
...
Eve cannot exist in isolation, so we need a way to communicate with the outside world. We've done some work on this front, adding the ability to read/write files, handle JSON (opening up the ability to interact with any JSON compatible API), and handle HTTP requests. 

What's nice about our solution here is that interacting with the outside world doesn't look or feel any different than interacting within Eve; you just match and modify records. If you've been using Eve, then you're already interacting with the browser through changing the DOM and handling events like clicks and keypresses. 

Eric has been working on a little [HTTP file server][1.4.1] written in Eve. It's still a work in progress, but the functionality to accept requests and serve files is already working. Take a look at how an HTTP request is handled: 
...
You'll see here that we are interacting with both an external `#http-request` and the file system through `#path`. Again, what's notable is that despite interacting with data external to Eve, the code is written as if you were working entirely within the boundaries of Eve.
...
Whereas other literate programming implementations require a "tangle" step to turn a literate program into a compilable program, Eve's semantics mean a `*.eve` file can be rendered as markdown or executed by Eve without any additional compilation step. We accomplish this by being [CommonMark][1.3.5] compatible; Eve programs are written just as you would write any markdown document, but code blocks containing Eve code are actually executable.
...
I'm especially excited by the engagement we've seen from the community so far. It's hard to know exactly how many people are following our development and using Eve, but so far we've received comments and feedback from Sydney, Copenhagen, Moscow, Finland, Hong Kong, and many other places.
