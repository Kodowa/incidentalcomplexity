...
Building under that, we use a reserved word `without`, an alias for the anti-join operator. The adjacent blocks `width` and `height` mean `person.width` and `person.height`. So the overall query will select all entities with a people tag and an attribute age, that do not have width or height attributes.
