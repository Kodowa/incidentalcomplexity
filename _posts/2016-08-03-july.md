...
Below is an analogue [clock in Eve](https://github.com/witheve/Eve/blob/19b91747ef8371e55a3a7b2288b19dcdba5c0d2d/examples/clock.eve) that reacts to changes in `#time`, which is an object in the system that represents the current time with `hours`, `minutes`, and `seconds`, attributes.
...
This is a fun example, but Unicode can be important in certain domains where notaion is standardized (e.g. the number π in math), but moreover it's crucial for making Eve usable in languages other than English.
